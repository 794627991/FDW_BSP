...
{
    WORD a, b, c, d, e, f, g, h, i, j, t1, t2, m[64], xx;
    // initialization
    ...
    //m[i] = ((WORD)data[j] << 24) | ((WORD)data[j + 1] << 16) | (data[j + 2] << 8) | (data[j + 3]);
    for (; i < 64; ++i)
...
//数据，加密的结果，要加密的数据长度
void sha256_deal(const BYTE *data, BYTE *hash, size_t len) //处理
{
    SHA256_CTX ctx;
    sha256_init(&ctx);
