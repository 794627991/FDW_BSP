...
*		V1.0.2  2021-2-1    高屹    API_EPROM操作支持到24V10，地址可大于0xffff
*
...
*	形    参: 无
*	返 回 值: 无
*	备    注：弱定义函数可重写，I2CStartAdr需要等于结构体成员的第一个变量或者结构体名，调用下述函数即可
*             API_IIC_SAVE_TO(name, buf) // 将 buf 中数据存入 结构体成员变量 name 
*             API_IIC_READ_TO(name, buf) // 将 结构体成员变量 name 数据读出到 buf 
*             API_IIC_SAVE(name) // 直接将结构体成员变量 name 数据存入到EEPROM
*             API_IIC_READ(name) // 从EEPROM中将数据读出到 结构体成员变量 name 
*********************************************************************************************************
*/
__weak void API_SaveAdr_Init(void)
{
    I2CBaseAdr = 0;
    I2CStartAdr = (uint32_t)(&epromstart);
}
/*
*********************************************************************************************************
*	函 数 名: __API_EPROM_Save
*	功能说明: 使用结构体存储方式 存
*	形    参: base：地址
*             *Buf：数据缓存
*             len：长度
