...
*/
extern void lcd_send(int x,int y);
extern void lcd_re_send(int x,int y);
static unsigned char lcdxbuf[30];
static unsigned char lcdybuf[16];
...
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
  ...
  /* Read from hardware ... Adapt to your system */
  {
		...
		if( ((lcdxbuf[x/8]&(1<<(x%8))) == ((1<<(x%8))))  &&  ((lcdybuf[y/8]&(1<<(y%8))) == ((1<<(y%8))))  )
			PixelIndex = 1;
		else
			PixelIndex = 0;/* ... */
  }
